...
class RelationTest(TestCase):
    ...
    async def test_create_operator(self):
        async with aioduckdb.connect(TEST_DB) as conn:
            ...
            self.assertEqual(
                await r.fetchall(), [(1, "one"), (2, "two"), (3, "three"), (4, "four")]
            )
    ...
    async def test_join_types(self):
        async with aioduckdb.connect(TEST_DB) as conn:
            ...
            self.assertEqual(
                (
                    await (
                        await (await rel1.join(rel2, "i=j", "inner")).aggregate(
                            "count()"
                        )
                    ).fetchone()
                )[0],
            ...
            self.assertEqual(
                (
                    await (
                        await (await rel1.join(rel2, "i=j", "left")).aggregate(
                            "count()"
                        )
                    ).fetchone()
                )[0],
    ...
    async def test_fetchnumpy(self):
        ...
        async with aioduckdb.connect(TEST_DB) as con:
            await con.execute(
                f"CREATE table t AS SELECT range AS a FROM range({start}, {stop});"
            )
            ...
            with self.assertRaisesRegex(
                aioduckdb.ConversionException, "Conversion Error.*out of range.*"
            ):
