...
filename = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "data", "binary_string.parquet"
)
...
class TestParquet(TestCase):
        ...
        async with aioduckdb.connect(TEST_DB) as conn:
            res = await (
                await conn.execute_on_self(
                    "SELECT typeof(#1) FROM parquet_scan('" + filename + "') limit 1"
                )
            ).fetchall()
    ...
    async def test_scan_binary_as_string(self):
        async with aioduckdb.connect(TEST_DB) as conn:
            res = await (
                await conn.execute_on_self(
                    "SELECT typeof(#1) FROM parquet_scan('"
                    + filename
                    + "',binary_as_string=True) limit 1"
                )
            ).fetchall()
        ...
        async with aioduckdb.connect(TEST_DB) as conn:
            ...
            res = await (
                await conn.execute_on_self(
                    "SELECT typeof(#1) FROM parquet_scan('"
                    + filename
                    + "',binary_as_string=False) limit 1"
                )
            ).fetchall()
            self.assertEqual(res[0], ("BLOB",))
